...
- **`ReportGenerationError` cuando falla la generación del informe PDF:** si la orden "Informe" no puede renderizar o escribir el PDF (ruta no escribible, destino inválido), `flow.py` envuelve el fallo en `ReportGenerationError(RuntimeError)` conservando la causa original (`__cause__`) sin alterar la máquina de estados; el `Deliverable` permanece disponible para reintentar a otra ruta. Comportamiento comprobado en `tests/test_error_handling_local.py`.
- **Errores de ingesta tipificados por causa (parser/loader):** `src/article/parser.py` distingue PDF corrupto/ilegible (`pypdf.errors.PdfReadError` → `RuntimeError` con causa encadenada), fallo de E/S (`OSError`) y contenido no extraíble (`ValueError`); `src/data/loader.py` preserva `FileNotFoundError`, mantiene `ValueError` de extensión no soportada, y envuelve en `RuntimeError` con mensaje específico y causa encadenada los fallos de CSV malformado (`pd.errors.ParserError`), Excel dañado (`zipfile.BadZipFile` / `ValueError`), hoja inexistente (`KeyError`/`ValueError`), decodificación (`UnicodeDecodeError`) y E/S (`OSError`). Comportamientos comprobados en `tests/test_error_handling_local.py`.
...
5. **No existen plantillas de prompts versionadas para el motor (ni en `src/llm/` ni en un directorio `prompts/`).** Verificado: `ls src/llm/` solo contiene `base.py` y `__init__.py`; `grep -RIn "prompt" src --include="*.py"` solo retorna las dos firmas `generate(self, prompt: str)` vacías; `grep -RIn "template|jinja|system.*role" src docs` → 0 hits. Los únicos prompts activos del repositorio son de **nivel agente** (no del motor): `.opencode/agents/paperstats.md` (subagente, con contrato de salida y ejemplos desde v2) y `AGENTS.md` (nivel sesión). Su historial e iteración están documentados en `docs/prompts_evolucion.md`.

6. **Crear plantillas de prompts para el motor en este momento sería introducir artefactos sin consumidor real.** No hay operación en P-FLOW ni en `RuleBasedLLMClient` que lea `prompts/v1` o `v2`; serían plantillas muertas, sin llamada, sin test que las ejercite y sin métrica que las compare, violando `AGENTS.md:172` y `docs/roadmap.md:112-115`. El prompt de subagente `.opencode/agents/paperstats.md`, en cambio, sí tiene consumidor real (el runtime de opencode al invocar el agente) y por eso sí se itera y documenta (`docs/prompts_evolucion.md`).

7. **Por tanto, la ingeniería de prompts a nivel motor queda declarada como NO APLICABLE a la arquitectura actual.** La evaluación del criterio debe considerar que no hay componente generativo que consuma prompts del motor, tal como `§12` documenta "Actualmente no existe un modelo LLM generativo integrado" y `§10 llm/` "No participa en las decisiones estadísticas de P-FLOW". A nivel agente, el subagente `.opencode/agents/paperstats.md` mantiene contrato de salida, restricciones y ejemplos, con iteración v1 → v2 trazable.
